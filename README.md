...
**Why processed over confirmed for the primary blockhash source:** The Solana documentation recommends `confirmed` as the preferred commitment level for `getLatestBlockhash` in most applications, and that recommendation is sound. A `confirmed` block has received votes from a supermajority of stake, a greater than 99.9% chance of reaching finality, making it essentially canonical the moment it is confirmed. A confirmed blockhash is one to two slots behind processed in normal conditions, which maps directly to our measured processed→confirmed delta of 213–840 ms. For a wallet, a DEX frontend, or any application that can absorb that latency, `confirmed` is the correct default.

MEV bots and latency-sensitive bundle infrastructure make the opposite call deliberately. Copilot's primary blockhash source is the Geyser stream's `BlockMeta` events at `processed` commitment, which means the block the leader just produced, aged by milliseconds rather than slots. For Jito bundle submission, where the 150-slot validity window starts shrinking from the moment the blockhash is fetched, and where the next Jito-connected leader may be 5–20 slots away, every slot of additional freshness directly extends the submission runway. The risk, that a `processed` block gets forked before becoming canonical, invalidating any transaction built on its blockhash, is real theoritically. Practically, Solana's observed fork rate is well under 0.1% of slots, and the entire production MEV ecosystem on mainnet accepts this risk because starting with a blockhash that is already one slot old when fetched is a meaningfully worse tradeoff for racing to a leader.

The most convenient choice depends on what you are optimizing for. `processed` maximizes validity runway at the cost of a small, measurable fork risk. `confirmed` minimizes fork risk at the cost of 400–800 ms of staleness at the moment of fetch. Copilot makes this call explicitly for a Jito-bundle submission pipeline, not because the docs are wrong, but because the docs are giving advice for a different problem.
